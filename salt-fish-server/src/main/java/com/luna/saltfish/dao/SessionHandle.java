...
public class SessionHandle {
    ...
    public boolean insert(Session session) throws SQLException {
        ...
        return JdbcTemplate.update(sql, session.getKey(), session.getUserId()) > 0;
    }
     ...
     * 更新Session Key
     */
    public boolean updateKey(int userId, String newKey) throws SQLException {
        String sql = "UPDATE sessionkey SET session_key=? WHERE user_id=?";
        return JdbcTemplate.update(sql, newKey, userId) > 0;
    }
    ...
    public boolean deleteByUserId(int userId) throws SQLException {
        String sql = "DELETE FROM sessionkey WHERE user_id=?";

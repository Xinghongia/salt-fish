...
public class ShopCartHandle {
    ...
    public List<Goods> findGoodsByUser(User user) throws Exception {
        String sql =
            "select id, num, content, type_id, image, producter_id, price, create_date, name, status from `goods` where id=any(SELECT goods_id from `shoppingcart`  where user_id=?) order by create_date desc, id desc";
     ...
     * @throws Exception
     */
    public List<Goods> findGoodsByUserPaged(User user, int limitMin, int perPage, String sort) throws Exception {
        String orderBy = "create_date desc, id desc";
        if ("price_asc".equals(sort)) orderBy = "price asc, id desc";
        else if ("price_desc".equals(sort)) orderBy = "price desc, id desc";
        else if ("date_asc".equals(sort)) orderBy = "create_date asc, id desc";
        String sql = "select id, num, content, type_id, image, producter_id, price, create_date, name, status from goods where id=any(SELECT goods_id from shoppingcart where user_id=?) order by " + orderBy + " limit ?,?";
        return JdbcTemplate.query(sql, new GoodsHandle().getBeanListHandler(), user.getId(), limitMin, perPage);
    }

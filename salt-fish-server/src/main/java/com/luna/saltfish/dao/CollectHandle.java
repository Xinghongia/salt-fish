...
public class CollectHandle {
     ...
     * @throws Exception
     */
    public List<Goods> findGoodsByUser(User user, PageResult num, int limitMin, int perPage, String sort) throws Exception {
        ...
        String sql =
            "select id, num, content, type_id, image, producter_id, price, create_date, name, status from `goods` where id = any(SELECT goods_id from `collect`  where user_id=?) order by " + orderBy + " limit ?,?";

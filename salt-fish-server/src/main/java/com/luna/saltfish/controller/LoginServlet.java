            ...
            if (userHandle.findByEmail(inputEmail) != null) {
                ...
                if (user.getPwd().equals(pass)) {
                    ...
                    if (autoLogin != null && autoLogin.equals("on")) {
                        Cookie cookieE = new Cookie(CookieNameConstant.LOGIN_EMAIL, inputEmail);
                        ...
                        if (sessionHandle.getSession(user.getId()) == null) {
                            ...
                            sessionHandle.updateKey(user.getId(), inputEmailMd5);
                        }
                    } else {
                        // 未勾选自动登录，清除旧的session key
                        sessionHandle.deleteByUserId(user.getId());
                        // 清除cookie
                        Cookie cookieE = new Cookie(CookieNameConstant.LOGIN_EMAIL, "");
                        cookieE.setMaxAge(0);
                        cookieE.setPath("/");
                        response.addCookie(cookieE);
                    }

...
<head>
    ...
    <style>
        .collect-item{display:flex;gap:var(--space-lg);background:var(--bg-card);border-radius:var(--border-radius-lg);padding:var(--space-lg);margin-bottom:var(--space-md);box-shadow:var(--shadow-sm);border:1px solid var(--border-color)}
        .collect-item .item-img{width:100px;height:100px;border-radius:var(--border-radius);object-fit:cover;flex-shrink:0}
    </style>
</head>
<body>
    <jsp:include page="/common/header.jsp" />
    <jsp:include page="/common/personal-sidebar.jsp">
        <jsp:param name="activeTab" value="like"/>
    </jsp:include>

    <div style="display:flex;align-items:center;margin-bottom:var(--space-xl)">
        <div>
            <h1 style="font-size:1.4rem;display:flex;align-items:center;gap:8px"><i data-lucide="heart" class="icon-lg"></i> 我的收藏</h1>
            <p style="color:var(--text-secondary);font-size:0.85rem;margin-top:2px">共 ${total} 件商品</p>
        </div>
                <div class="sort-wrap">
            <label>排序</label>
            <select onchange="location.href=this.value">
                <option value="${pageContext.request.contextPath}/collect/check?pn=${pn}&userId=${viewUser.id}&sort=date_desc" ${sort == 'date_desc' ? 'selected' : ''}>时间最新</option>
                <option value="${pageContext.request.contextPath}/collect/check?pn=${pn}&userId=${viewUser.id}&sort=date_asc" ${sort == 'date_asc' ? 'selected' : ''}>时间最早</option>
                <option value="${pageContext.request.contextPath}/collect/check?pn=${pn}&userId=${viewUser.id}&sort=price_asc" ${sort == 'price_asc' ? 'selected' : ''}>价格低→高</option>
                <option value="${pageContext.request.contextPath}/collect/check?pn=${pn}&userId=${viewUser.id}&sort=price_desc" ${sort == 'price_desc' ? 'selected' : ''}>价格高→低</option>
            </select>
        </div>
    </div>
    ...
    <c:choose>
        <c:when test="${not empty goodsList}">
            <c:forEach var="goods" items="${goodsList}">
                <div class="collect-item" id="collect-${goods.id}">
                    ...
                    <div style="flex:1">
                        <div style="font-size:1rem;font-weight:600;margin-bottom:var(--space-xs)">
                            <c:choose>
                                <c:when test="${goods.status == 2}"><span class="badge badge-success" style="font-size:0.7rem">出售中</span></c:when>
                    ...
                    <div style="display:flex;align-items:flex-end">
                        <button class="btn btn-danger btn-sm" onclick="removeCollect(${goods.id})"><i data-lucide="trash-2" class="icon-sm"></i> 移除</button>
                    </div>
                </div>
            </c:forEach>
        </c:when>
        <c:otherwise>
            <div class="empty-state"><div class="empty-icon"><i data-lucide="heart" style="width:56px;height:56px;color:var(--text-light)"></i></div><h3>还没有收藏任何商品</h3><a href="${pageContext.request.contextPath}/index" class="btn btn-primary">去逛逛</a></div>
        </c:otherwise>
    </c:choose>
    ...
    <div class="pagination" style="margin-top:var(--space-xl)">
        ...
        <c:forEach begin="1" end="${maxPage}" var="i">
            <a href="${pageContext.request.contextPath}/collect/check?pn=${i}&userId=${viewUser.id}&sort=${sort}" class="page-btn ${i == pn ? 'active' : ''}">${i}</a>
        </c:forEach>
    ...
    <script>
        function removeCollect(goodsId) {
            SaltFish.ajax('api/removeCollect?goodsId=' + goodsId, {
                callback: function(resp) {
                    if (resp && resp.indexOf('success') >= 0) {
                        var el = document.getElementById('collect-' + goodsId);
                        if (el) el.remove();
                        SaltFish.showToast('success', '已移除收藏');
